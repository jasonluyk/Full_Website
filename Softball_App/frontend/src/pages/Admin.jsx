...
export default function SoftballAdmin() {
  ...
  const [division, setDivision] = useState('10U : BB#2')
    ...
    <div style={{ maxWidth: 800, margin: '0 auto', padding: '32px 24px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 16 }}>
          ...
          <div>
            ...
            <input className="input" value={division} onChange={e => setDivision(e.target.value)}
              placeholder="e.g. 10U : BB#2 (leave blank for all)" />
          </div>
        </div>
          ...
          <button className="btn btn-primary" disabled={loading}
            onClick={() => handle(async () => {
              const r = await fetch('/softball/api/admin/softball/sync', {
                ...
                body: JSON.stringify({ trnid, division })
              })
      ...
      <div className="card" style={{ padding: 28, marginTop: 16 }}>
        ...
        <p style={{ color: 'var(--text-muted)', fontSize: 13, lineHeight: 1.8 }}>
          ...
          2. Optionally filter to a specific division (e.g. <code>10U : BB#2</code>)<br/>
          3. Click Sync — data loads in ~20 seconds (Playwright scrape)<br/>
          4. Page auto-refreshes every 2 minutes during the tournament
        </p>
      </div>
    </div>
  )
