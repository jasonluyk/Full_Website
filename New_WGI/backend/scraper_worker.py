...
def fetch_wgi_scores_simple(show_id):
    """
    Fetches WGI scores using plain requests — no browser needed.
    Returns list of {Guard, Class, Score} dicts.
    """
    import requests as req
    url = f"https://www.wgi.org/scores/color-guard-score-event/?ShowId={show_id}"
    try:
        resp = req.get(url, headers={"User-Agent": USER_AGENT}, timeout=20)
            ...
            for row in table.find_all('tr'):
                ...
                if len(cols) >= 3:
                    ...
                    try:
                        score = float(score_text)
                        if guard:
                            records.append({"Guard": guard, "Class": current_class, "Score": score})
                    except ValueError:
                        continue
        return records
    except Exception as e:
...
def poll_round_for_scores(show_name, show_id, round_label, is_worlds=False, session_id=None):
    """
    Polls WGI every 2 minutes until at least 1 score appears.
    Uses plain requests — no browser needed.
    ...
    for attempt in range(max_attempts):
        records = fetch_wgi_scores_simple(show_id)
        if records:
            ...
            print(f"  ⏳ [POLLER] No scores yet for {round_label} (attempt {attempt+1})")
    ...
    while True:
        ...
        try:
            ...
            for row in soup.find_all('tr'):
                ...
                score_td = row.find('td', class_='event-scores')
                if not name_td or not score_td:
                    continue
                show_name = name_td.get_text(strip=True)
                link = score_td.find('a', href=True)
                if link and 'ShowId=' in link['href']:
                    show_id_val = link['href'].split('ShowId=')[-1].strip()
                    show_id_map[show_name.lower().strip()] = show_id_val
